<nav class="navbar navbar-expand-lg  py-2  bg-nav-c  ">
    <div class="container">
        ...
        @endphp
        <a class="navbar-brand buk-29" href="{{route('home')}}"> <img src="{{ $setting ->logo ? request()->root().'/'. $setting ->logo : asset('public/'.'images/logo.svg') }}" alt=""></a>
        ...
        <div class="collapse navbar-collapse bar-sm" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto cairo ml-2 kml-e">
                ...
                @auth
                        @if(auth()->user()->nots->where('read',0)->count() > 0)
                            <li class="nav-item mx-1">
                ...
                @if(auth()->guard('employee')->check())
                    @if(auth()->guard('employee')->user()->nots->where('read',0)->count() > 0)
                        <li class="nav-item mx-1">
                            <a class="nav-link position-relative d-inline-block" href="{{route('nots.index')}}">
                                <i class="fas fa-bell my-nfx"></i>
                                <span class="btn btn-danger btn-sm not-num">{{auth()->guard('employee')->user()->nots->where('read',0)->count()}}</span>
                            </a>
                        </li>
                    @endif
                    <li class="nav-item mx-1">
                        <a class="nav-link" href="{{route('employee.account')}}">
