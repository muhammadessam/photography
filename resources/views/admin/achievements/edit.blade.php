...
@section('content')
    <div class="container p-4">
        <h3 class="col-12 text-right">تعديل انجاز</h3>
    </div>
        ...
        <div class="card-body">
            <form action="{{route('admin.achievements.update', $achievement)}}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" value="{{$achievement->title}}" required class="form-control">
                </div>
                <div class="form-group">
                    <label for="number">العدد</label>
                    <input type="text" id="number" value="{{$achievement->number}}" required name="number"class="form-control">
                </div>
                <div class="form-group">
                    <label for="icons">أيقونة</label>
                    <div class="d-flex align-items-center">
                        <div class="m-1">
                            <i class="fa fa-{{ $achievement->icon }}" data-icon="{{ $achievement->icon }}" id="icon-preview"></i>
                        </div>
...
@section('javascript')
    ...
    <script>
        ...
        faIcons.forEach(function(icon, index){
            var currentIcon = $('#icon-preview').data('icon')
            if (currentIcon == icon) {
                $('#icons').append('<option value="'+icon+'" selected>'+icon+'</option>')
            } else {
